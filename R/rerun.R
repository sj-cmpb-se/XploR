...
#' @param top_rows Data frame or tiblle. Top allelic calls generated by inicial run.
#'
...
if( 'mu' %in% colnames(models)){
  ...
  final_model <- models %>%
    dplyr::filter( rho >= min(purity) & rho <= max(purity) ) %>%
    dplyr::arrange( abs( ploidy - 2),
                    desc( rho ),
                    dis_integer_CN)

  final_model <- top_rows %>%
    dplyr::filter( rho >= (final_model$rho[1] - 0.05) & rho <= (final_model$rho[1] + 0.05 ) ) %>%
    dplyr::filter( mu >= (min(dicovsf) - 0.05) & mu <= (max(dicovsf)+0.05)) %>%
    dplyr::arrange( desc(rho), abs( mu -1) )
